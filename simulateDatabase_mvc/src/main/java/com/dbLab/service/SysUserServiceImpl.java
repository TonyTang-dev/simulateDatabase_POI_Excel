...
public class SysUserServiceImpl implements ISysUserService {
	...
	public selectEntity operateSelectDatabase(String operation){
		...
		if(operationStr.length == 4 || operationStr.length == 5) {
	...
	public boolean operateDatabase(String operation){
		...
		switch (mainOpe){
			...
			case "use":
				return sysDao.selectDatabase(globalCmd.databasePath+operationStr[1]);
			/*insert增加数据*/
			case "insert":
			...
			case "update":
				if(operationStr.length == 4){
					return sysDao.updateTable(operationStr[1], 1,null, operationStr[3].split("="));
				}
				else{
					return sysDao.updateTable(operationStr[1], 2, operationStr[5].split("="), operationStr[3].split("="));
				}
		}
