...
//				cell.setCellValue("数据表数");     //设置cell浮点类型的值
//				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
//				cell=currow.createCell(1);
//				cell.setCellValue("0");   //设置cell字符类型的值
//				cell.setCellType(HSSFCell.CELL_TYPE_STRING);
...
//		sheet = conn.getSheet("系统表");
//		HSSFRow tableRow = sheet.getRow(0);
		HSSFCell cell=null;

		sheet = conn.createSheet(tableName);
...
//		写入字段
		...
		try {
			/*调用表数据插入系统表*/
			opeIndex.insertIndexHead(conn, conn.getSheetIndex(sheet), tableName);
...
//		创建行
		...
		try {
			...
			try {
				...
				/*调用表数据插入系统表*/
				opeIndex.insertIndex(conn, conn.getSheetIndex(sheet), values[0]+"-"+String.valueOf(rowIndex));
...
//				}
//			}
			return opeIndex.select(tableName, params[1]);
		}
		return null;
	}
	/**
